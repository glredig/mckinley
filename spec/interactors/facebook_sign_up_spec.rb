...
RSpec.describe FacebookSignUp do
  ...
  context 'successful sign up with token' do
    before do
      ...
      allow(interactor_token).to receive(:build_user_from_facebook).and_return(user)
      allow(user).to receive(:save).and_return(true)
      allow(user).to receive(:facebook).and_return(facebook)
    ...
    it { expect(interactor_token).to have_received(:build_user_from_facebook) }
    it { expect(user).to have_received(:save) }
    it { expect(user).to have_received(:facebook) }
  ...
  context 'failed sign up with token' do
    ...
    it { expect(interactor_token).to have_received(:build_user_from_facebook) }
    it { expect(user).to have_received(:save) }
    it { expect(user).to have_received(:errors) }
